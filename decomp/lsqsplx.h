...
void gpFISTA(const Matrix<T>& A, const Vector<T>& b, Vector<T>& xCurr, const T L0 = 1.0, const T eta = T(1.0/0.7), const T epsilon = T(1e-6), const bool warm = false) {
  ...
  while(true) {
    ...
    while(true) {
      ...
      yDelta.copy(yPrev);
      yDelta.sub(yCurr);
      T fDelta = 0.5*AyPrev.nrm2sq() - 0.5*AyCurr.nrm2sq() + c.dot(yDelta);
      if(fDelta + 0.5*L*yDelta.nrm2sq() < gradPrev.dot(yDelta)) {
        L *= eta;
      } else {
        break;
    ...
    xDelta.copy(xPrev);
    xDelta.sub(xCurr);
    T fXDelta = 0.5*AxPrev.nrm2sq() - 0.5*AxCurr.nrm2sq() + c.dot(xDelta);
    if(fXDelta < epsilon)
      break;
...
void activeSet(const Matrix<T>& M, const Vector<T>& b, Vector<T>& xCurr, const T lambda2 = T(1e-5), const T epsilon = T(1e-5), bool warm = false) {
  ...
  Vector<int> NASet(L);
  NASet.set(-1);
  int* const pr_NASet = NASet.rawX();
  Vector<bool> NAMask(p);
  NAMask.set(false);
  int na;
  Vector<T> xRed(L);
  T* const pr_xRed = xRed.rawX();
  Vector<T> cRed(L);
  T* const pr_cRed = cRed.rawX();
  Matrix<T> MRed(m,L);
  ...
  T* const pr_gRed = gRed.rawX();
  Vector<T> MRedxRed(m);
  T* const pr_MRedxRed = MRedxRed.rawX();
