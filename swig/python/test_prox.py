...
def test_FistaFlat():
    ...
    # param.size_group=10;
    (W, optim_info) = Xtest1('spams','spams.FistaFlat(Y,X,W0,param,True)',locals())
...
###
    ...
    param['regul'] = 'l2';
    (W, optim_info) = Xtest1('spams','spams.FistaFlat(Y,ssp.csc_matrix(X),W0,param,True)',locals())
...
####    
    ...
    (W, optim_info) = Xtest1('spams','spams.FistaFlat(Y,x1,W01,param,True)',locals()) # adds a column of ones to X for the intercept,True)',locals())
    print 'mean loss: %f, mean relative duality_gap: %f, number of iterations: %f' %(np.mean(optim_info[0,:]),np.mean(optim_info[2,:]),np.mean(optim_info[3,:]))
    ...
    x1 = np.asfortranarray(np.concatenate((X,np.ones((X.shape[0],1))),1))
    W01 = np.asfortranarray(np.concatenate((W0,np.zeros((1,W0.shape[1]))),0))
    (W, optim_info) = Xtest1('spams','spams.FistaFlat(Y,x1,W01,param,True)',locals())
    print 'mean loss: %f, number of iterations: %f' %(np.mean(optim_info[0,:]),np.mean(optim_info[3,:]))
    param['pos'] = False
...
def test_ProximalFlat():
    ...
    alpha = Xtest1('spams','spams.ProximalFlat(U,param,False)',locals())
