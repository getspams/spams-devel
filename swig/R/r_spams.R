...
# do.call does not work for matrix args of kind output
...
spams.SparseProject <- function(U,param) {
  m = nrow(U)
  n = ncol(U)
...
##  do.call(sparseProject,c(list(U,V),params))
...
spams.Lasso <- function(...,return_reg_path = FALSE) {
#  require('Matrix')
  paramlist = list('L' = -1,'lambda' = NULL,'lambda2' = 0.,
  ...
  if(length(args) == 3)
    ...
    if (length(args) == 4) {
      ...
      stop("Lasso : bad nb of arguments")
    }
  }
  X = args[1][[1]]
  D = args[2][[1]]
  if(! is.null(param[['mode']]))
    .verif_enum(param[['mode']],'constraint_type','mode in Lasso')
  params = .param_struct(paramlist,param)
  path = NULL
  x = NULL
  if(! is.null(q)) {
...
spams.FistaFlat <- function(Y,X,W0,param,return_optim_info = FALSE) {
  paramlist = list("numThreads" = -1, "max_it" = 1000,'L0' = 1.0,
    'fixed_step' = FALSE, 'gamma' = 1.5,'lambda' = 1.0,'delta' = 1.0,'lambda2' = 0.,
    ...
    'inner_weights' = c(0.),'eval' = FALSE,'size_group' = 1,
...
spams.ProximalFlat <- function(alpha0,param,return_val_loss = FALSE) {
  paramlist = list("numThreads" = -1,'lambda' = 1.0,'lambda2' = 0.,
...
###########  dictLearn ##################
.TrainDL <- function(X,param,return_model,model,in_memory) {
  # We can only have simple objects in the param list of .mycall
  
  paramlist = list("numThreads" = -1,"batchsize" =  -1,
    "K" =  -1,'lambda' =  NULL,'lambda2'= 10e-10,
    'iter' = -1,'t0' = 1e-5,'mode' = 'PENALTY',
    'posAlpha' = FALSE,'posD' = FALSE,'expand' = FALSE,
    'modeD' = 'L2','whiten' = FALSE,'clean' = TRUE,
  ...
  if(! is.null(param[['modeD']]))
    .verif_enum(param[['modeD']],'constraint_type_D','modeD in TrainDL')
  if(! is.null(param[['mode']]))
    .verif_enum(param[['mode']],'constraint_type','mode in TrainDL')
  params = .param_struct(paramlist,param)
  if (is.null(model)) {
...
spams.TrainDL <- function(X,param,model= NULL,return_model= FALSE) {
  return (.TrainDL(X,param,return_model,model,FALSE))
}

spams.TrainDL_Memory <- function(X,param) {
  return (.TrainDL(X,param,FALSE,NULL,TRUE))
}
